...
const authorCreateValidator = async function (req, res, next) {
    ...
    if(dbData.length!=0){  //check if any document is returned from DB call
        return res.status(400).send({status: false, message: "Bad Request. This email already exist. Please enter another email"})
        }   
...
const loginvalidation = async function (req, res, next) {
    try {
        ...
        if ((body.email.length == 0) || (body.password.length == 0)) {
            return res.status(400).send({ status: false, message: "This fields are required. Cannot be empty" })
        }
...
const blogCreateValidator = async function (req, res, next) {
    ...
    try {
        ...
        if ((body.title.trim().length) < 4) {
            return res.status(400).send({ status: false, message: "Bad Request. Title must be atleast 4 character" })
        }

        // check if enterd body length is atleast 5 character
        if ((body.body.trim().length) < 4) {
            return res.status(400).send({ status: false, message: "Bad Request. Body must have some words" })
        }

        // check if entered body, category and authorId is blank or empty
        if ((body.category.trim().length == 0) || (body.authorId.trim().length == 0)) {
            return res.status(400).send({ status: false, message: "Bad Request. All fields must have a valid entry" })
        }
        ...
        if (("tags" in body) && (body.tags.length == 0)) {
            return res.status(400).send({ status: false, message: "Bad Request. Tags cannot be empty" })
        }
        ...
        if (("subcategory" in body) && (body.subcategory.length == 0)) {
            return res.status(400).send({ status: false, message: "Bad Request. Subgategory cannot be empty" })
        }
        ...
        if (!searchAuthId) { //if author id not found in DB, cannot create a blog
            return res.status(404).send({ message: "Resource Not found. Please create an account" })
        }
...
const updatevalidation = async function (req, res, next) {
    try {
        ...
        if (("title" in body) && (body.title.trim().length == 0)) {
            return res.status(400).send({ status: false, message: "Bad Request. Title cannot be empty" })
        }
        ...
        if (("body" in body) && (body.body.trim().length == 0)) {
            return res.status(400).send({ status: false, message: "Bad Request. Body cannot be empty" })
        }
